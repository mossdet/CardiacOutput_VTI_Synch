"""
Get the peaks from the filtered heart rate signals from both datasets and plot them.
Peaks should have a distance and prominence of 3
...
def plot_explorative_peaks(mntr, wrbl, mov_avg_period_s):
    ...
    fig.suptitle('Uncorrected Wearable Timestamps \n (Explorative Peaks)')
    ...
    for stage in ['Baseline', 'Stage 1', 'Stage 2']:
        ...
        ax2.set_ylabel("10s Avg Heart Rate (BPM)")
        ax2.set_xlim(min(timeVecFP), max(timeVecFP))
        ax2.set_xlabel('Time (s)')
    ...
    plt.tight_layout()
    figFilename = get_images_destination_path() + '2_Explorative_Peaks.png'
